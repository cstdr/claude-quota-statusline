...
- **多模型拆解**：API 返回 `model_remains[]` 含 `general` / `video` 等多个 model_name，现在只看了 `general`。`video` 是 count-based（5h 3 次、周 21 次），需要另一种显示格式。
- **配额 sparkline**：持久化 6 个数据点拼 `▁▂▃▅▆▇` 显示趋势。已经实现了，但还没合并到主线。

下次再写新东西前会先打 5+3 框架自检（"用户 5 维 + 我们 3 维"），避免再做出"技术上对但没人用"的东西。
